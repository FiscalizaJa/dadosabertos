...
const base_url = `http://localhost:${port}/senado`;
 ...
 */
const invalid_urls = [
    `${base_url}/senators?orderby=name&order=; SELECT *`, // a (very dumb) try to do SQL injection
    `${base_url}/senators?orderby=nome&order=asc`, // misspelled orderby
    `${base_url}/senators?itens=0`, // 0 itens is not allowed
    `${base_url}/senators?itens=10&page=0`, // page 0 is not allowed
    
    `${base_url}/senators/5672/expenses?year=${current_date.getFullYear() + 1}`, // year higher than actual date
    `${base_url}/senators/5672/expenses?year=2008`, // year minor than 2009
    `${base_url}/senators/5672/expenses?year=2024&month=13`, // month out of range 1-12
    `${base_url}/senators/5672/expenses?itens=0`, // 0 itens is not allowed
    `${base_url}/senators/5672/expenses?page=0`, // page 0 is not allowed.
